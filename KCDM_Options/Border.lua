...
local function BuildBorders(subPage, page)
    local rc, sc = UI.MakeSubPageScroll(subPage, "KCDM_Border_BordersScrollFrame")
    local yOff = 0

    local borderHeader = UI.CreateHeader(rc, L["Border Settings"])
    borderHeader:SetPoint("TOPLEFT", 0, yOff); yOff = yOff - 30

    local lblDropdown = rc:CreateFontString(nil, "ARTWORK", "KCDM_Font14")
...
local function BuildLook(subPage, page)
    local rc, sc = UI.MakeSubPageScroll(subPage, "KCDM_Border_LookScrollFrame")
    local yOff = 0
    ...
    local swipeColorLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
    ...
    local cooldownColorHeader = UI.CreateHeader(rc, L["Cooldown Color"])
    cooldownColorHeader:SetPoint("TOPLEFT", 0, yOff)
    yOff = yOff - 35
    ...
    local cooldownColorInit = CDM.db.cooldownIconColor or { r = 1, g = 1, b = 1, a = 1 }
    local cooldownColorPicker = UI.CreateSimpleColorPicker(rc, cooldownColorInit, function(r, g, b)
        CDM.db.cooldownIconColor = { r = r, g = g, b = b, a = 1 }
        API:Refresh("STYLE")
    end, false)
    cooldownColorPicker:SetPoint("LEFT", cooldownColorLabel, "RIGHT", 6, 0)
    yOff = yOff - 35
