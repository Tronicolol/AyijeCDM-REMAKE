...
StaticPopupDialogs["KCDM_CONFIRM_DELETE_BAR_GROUP"] = {
    text = "",
    button1 = YES,
    button2 = NO,
    OnAccept = function()
        local fn = StaticPopupDialogs["KCDM_CONFIRM_DELETE_BAR_GROUP"]._pendingDelete
        if fn then fn() end
    end,
    timeout = 0,
    whileDead = true,
...
local function CreateBarsTab(page)
    ...
    local leftScroll = CreateFrame("ScrollFrame", "KCDM_BarGroupsLeftScroll", page, "ScrollFrameTemplate")
    ...
    local function CreateRenderHelpers(rc)
        ...
        function h.SectionHeader(text)
            ...
            firstSection = false
            local fs = rc:CreateFontString(nil, "ARTWORK", "KCDM_Font18")
        ...
        function h.Label(text)
            local fs = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
            fs:SetText(text)
            fs:SetPoint("TOPLEFT", 0, yOff)
            yOff = yOff - 22
            return fs
    ...
    local function RenderSpellOverrideSettings(rc, gd, groupIndex, spellID)
        ...
        local barColorLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
        ...
        local bgColorLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
        ...
        nameBox:SetFontObject("KCDM_Font14")
        ...
        h.AddYOff(-26)
        local hint = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font12")
    ...
    local function ConfigureHeaderRow(widget, groupIndex, groupData, isExpanded, isSelected, yOff)
        ...
        widget.deleteBtn:SetScript("OnClick", function()
            StaticPopupDialogs["KCDM_CONFIRM_DELETE_BAR_GROUP"].text = string.format(
                L["Delete bar group '%s'?"],
                groupData.name or ("Group " .. groupIndex))
            StaticPopupDialogs["KCDM_CONFIRM_DELETE_BAR_GROUP"]._pendingDelete = function()
            ...
            StaticPopup_Show("KCDM_CONFIRM_DELETE_BAR_GROUP")
        end)
    end
