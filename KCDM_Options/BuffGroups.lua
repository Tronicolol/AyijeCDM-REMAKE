...
StaticPopupDialogs["KCDM_CONFIRM_DELETE_GROUP"] = {
    text = "",
    button1 = YES,
    button2 = NO,
    OnAccept = function()
        local fn = StaticPopupDialogs["KCDM_CONFIRM_DELETE_GROUP"]._pendingDelete
        if fn then fn() end
    end,
    timeout = 0,
    whileDead = true,
...
local function CreateBuffGroupsTab(page)
    ...
    local leftScroll = CreateFrame("ScrollFrame", "KCDM_BuffGroupsLeftScroll", page, "ScrollFrameTemplate")
    ...
    local rightPlaceholder = rightPanel:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
    ...
    local function BuildOverrideSection(rc, yOff, spellID, groupIndex, existingOv, ensureOv, defaults, placeholderOpts, isCustomBuff)
        yOff = yOff - 10
        local overrideHeader = rc:CreateFontString(nil, "ARTWORK", "KCDM_Font18")
        ...
        if ttsChecked then
            ...
            if ttsOnShowEnabled then
                ...
                ttsShowBox:SetFontObject("KCDM_Font14")
                ...
                local ttsShowHint = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font12")
            ...
            if ttsOnHideEnabled then
                ...
                ttsHideBox:SetFontObject("KCDM_Font14")
                ...
                local ttsHideHint = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font12")
    ...
    ShowSpellSettings = function(spellID, groupIndex)
        ...
        local spellName = rc:CreateFontString(nil, "ARTWORK", "KCDM_Font18")
        ...
        local borderLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
        ...
        local glowColorLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
        ...
        if isCustom then
            ...
            if not (cbEntry and cbEntry.triggerType) then
                ...
                local sidLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
                ...
                local durLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
                ...
                local cbStatusText = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font12")
    ...
    local ungroupedCacheMessage = leftChild:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
    ...
    ShowCustomBuffAddPanel = function(targetGroupIndex)
        ...
        if #templates > 0 then
            local quickLabel = rc:CreateFontString(nil, "ARTWORK", "KCDM_Font14")
            ...
            for _, tmpl in ipairs(templates) do
                ...
                local tName = tRow:CreateFontString(nil, "OVERLAY", "KCDM_Font12")
        ...
        yOff = yOff - 10
        local advLabel = rc:CreateFontString(nil, "ARTWORK", "KCDM_Font14")
        advLabel:SetPoint("TOPLEFT", 0, yOff)
        advLabel:SetText(L["Custom Spell"])
        UI.SetTextWhite(advLabel)
        yOff = yOff - 24

        local sidLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font12")
        sidLabel:SetPoint("TOPLEFT", 0, yOff)
        sidLabel:SetText(L["Spell ID:"])
        ...
        local durLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font12")
        durLabel:SetPoint("TOPLEFT", 0, yOff)
        durLabel:SetText(L["Duration (sec):"])
        ...
        local previewText = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font12")
        ...
        local statusText = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font12")
        ...
        yOff = yOff - 10
        local disclaimer = rc:CreateFontString(nil, "ARTWORK", "KCDM_Font14")
            ...
            for groupIndex, groupData in ipairs(groups) do
                ...
                h.deleteBtn:SetScript("OnClick", function()
                    ...
                    if spellCount > 0 then
                        local dialog = StaticPopupDialogs["KCDM_CONFIRM_DELETE_GROUP"]
                        dialog.text = string.format(
                            L["Delete group with %d spell(s)?"],
                            spellCount
                        )
                        dialog._pendingDelete = DoDelete
                        StaticPopup_Show("KCDM_CONFIRM_DELETE_GROUP")
                    else
                        DoDelete()
                    end
                end)
