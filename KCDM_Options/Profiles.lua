...
local function SetupDefaultProfileDropdown(dropdown)
    dropdown:SetDefaultText(KCDMDB.global.defaultProfile or "Default")
    dropdown:SetupMenu(function(_, rootDescription)
        local profiles = API:GetProfileList()
        local currentDefault = KCDMDB.global.defaultProfile or "Default"
        for _, name in ipairs(profiles) do
            rootDescription:CreateRadio(name, function()
                return currentDefault == name
            end, function()
                KCDMDB.global.defaultProfile = name
...
local function RefreshProfilesUI()
    ...
    if copyDropdown then
        SetupProfileDropdown(copyDropdown, true, function(name)
            StaticPopup_Show("KCDM_CONFIRM_COPY_PROFILE", name, nil, { name = name })
        end)
    end

    if deleteDropdown then
        SetupProfileDropdown(deleteDropdown, true, function(name)
            StaticPopup_Show("KCDM_CONFIRM_DELETE_PROFILE", name, nil, { name = name })
        end)
    end
...
local function CreateProfilesTab(page, tabId)
    ...
    newEditBox:SetFontObject("KCDM_Font14")
    ...
    local createStatus = newProfileRow:CreateFontString(nil, "ARTWORK", "KCDM_Font14")
    ...
    local copyDesc = page:CreateFontString(nil, "ARTWORK", "KCDM_Font14")
    ...
    renameEditBox:SetFontObject("KCDM_Font14")
    renameEditBox:SetText(API:GetActiveProfileName())
    ...
    local renameStatus = renameRow:CreateFontString(nil, "ARTWORK", "KCDM_Font14")
    ...
    resetBtn:SetScript("OnClick", function()
        StaticPopup_Show("KCDM_CONFIRM_RESET_PROFILE")
    end)
    ...
    for i = 1, numSpecs do
        ...
        local label = specSection:CreateFontString(nil, "ARTWORK", "KCDM_Font14")
