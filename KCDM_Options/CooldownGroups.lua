...
StaticPopupDialogs["KCDM_CONFIRM_DELETE_CD_GROUP"] = {
    text = "",
    button1 = YES,
    button2 = NO,
    OnAccept = function()
        local fn = StaticPopupDialogs["KCDM_CONFIRM_DELETE_CD_GROUP"]._pendingDelete
        if fn then fn() end
    end,
    timeout = 0,
    whileDead = true,
...
local function CreateCooldownGroupsPanel(subPage, page)
    ...
    local gridEmptyText = iconGridFrame:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
    ...
    local leftScroll = CreateFrame("ScrollFrame", "KCDM_CDGroups_LeftScroll", subPage, "ScrollFrameTemplate")
    ...
    ShowSpellSettings = function(spellID, groupIndex)
        ...
        do
            ...
            if showAura then
                ...
                local agcLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
                ...
                local abcLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
            ...
            if readyGlowEnabled then
                local rgcLabel = rc:CreateFontString(nil, "OVERLAY", "KCDM_Font14")
            ...
            for groupIndex, groupData in ipairs(groups) do
                ...
                h.deleteBtn:SetScript("OnClick", function()
                    ...
                    if spellCount > 0 then
                        local dialog = StaticPopupDialogs["KCDM_CONFIRM_DELETE_CD_GROUP"]
                        dialog.text = string.format(
                            L["Delete group with %d spell(s)?"],
                            spellCount
                        )
                        dialog._pendingDelete = DoDelete
                        StaticPopup_Show("KCDM_CONFIRM_DELETE_CD_GROUP")
                    else
                        DoDelete()
                    end
                end)
