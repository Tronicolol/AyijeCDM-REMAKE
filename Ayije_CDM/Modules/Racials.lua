...
local function UpdateIcon(frame, updateCooldowns, updateCharges)
    ...
    if updateCooldowns and frame.Icon then
        local isRealCooldown = itemCooldownActive == true
        if desatDurationObject and desatSpellID then
            isRealCooldown = CDM.IsOnRealCooldown(desatSpellID, desatIsChargeSpell) == true
        end
        ...
        elseif desatDurationObject then
            if not CDM.styleCache.disableCooldownDesat and isRealCooldown then
        ...
        if CDM.ApplyCooldownIconAppearance then
            CDM.ApplyCooldownIconAppearance(frame, nil, false, nil, isRealCooldown)
        end
    end
