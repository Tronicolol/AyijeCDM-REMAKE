...
local function CreateGlowTab(page, tabId)
    ...
    local scrollChild = UI.CreateScrollableTab(page, "AyijeCDM_GlowScrollFrame", 1050, 520)
    ...
    UI.SetupValueDropdown(
        ...
        function(value, label)
            ...
            if RepositionPandemicSections then
                RepositionPandemicSections(value)
            else
    ...
    -- Pandemic uses its own type and colour, but reuses the tuning values
    -- (frequency, particles, duration, etc.) from the glow type sections below.
    local pandemicHeader = UI.CreateHeader(scrollChild, L["Pandemic Glow"] )
    pandemicHeader:SetPoint("TOPLEFT", colorPicker, "BOTTOMLEFT", 0, -330)

    local pandemicToggle
    local pandemicTypeDropdown
    local pandemicColorPicker

    local function UpdatePandemicGlowControls()
        local enabled = CDM.db.pandemicGlowEnabled == true
        if pandemicTypeDropdown then pandemicTypeDropdown:SetEnabled(enabled) end
        if pandemicColorPicker then pandemicColorPicker:SetEnabled(enabled) end
    end

    pandemicToggle = UI.CreateModernCheckbox(
        scrollChild,
        L["Override Default Pandemic Glow"],
        CDM.db.pandemicGlowEnabled or false,
        function(checked)
            CDM.db.pandemicGlowEnabled = checked
            UpdatePandemicGlowControls()
            API:Refresh("STYLE")
        end
    )
    pandemicToggle:SetPoint("TOPLEFT", pandemicHeader, "BOTTOMLEFT", 0, -15)

    local pandemicTypeLabel = scrollChild:CreateFontString(nil, "ARTWORK", "AyijeCDM_Font14")
    pandemicTypeLabel:SetText(L["Glow Type"])
    pandemicTypeLabel:SetPoint("TOPLEFT", pandemicToggle, "BOTTOMLEFT", 0, -15)

    pandemicTypeDropdown = CreateFrame("DropdownButton", nil, scrollChild, "WowStyle1DropdownTemplate")
    pandemicTypeDropdown:SetPoint("TOPLEFT", pandemicTypeLabel, "BOTTOMLEFT", 0, -10)
    pandemicTypeDropdown:SetWidth(200)
    pandemicTypeDropdown:SetDefaultText(UI.GetOptionLabel(glowTypeOptions, CDM.db.pandemicGlowType or "button", L["Button Glow"]))

    UI.SetupValueDropdown(
        pandemicTypeDropdown,
        glowTypeOptions,
        function() return CDM.db.pandemicGlowType or "button" end,
        function(value, label)
            CDM.db.pandemicGlowType = value
            pandemicTypeDropdown:SetDefaultText(label)
            API:Refresh("STYLE")
        end
    )

    pandemicColorPicker = UI.CreateColorSwatch(scrollChild, L["Glow Color"], "pandemicGlowColor", "STYLE")
    pandemicColorPicker:SetPoint("TOPLEFT", pandemicTypeDropdown, "BOTTOMLEFT", 0, -12)

    UpdatePandemicGlowControls()

    local pandemicDisplayHeader = UI.CreateHeader(scrollChild, L["Pandemic Display"])
    pandemicDisplayHeader:SetPoint("TOPLEFT", pandemicColorPicker, "BOTTOMLEFT", 0, -25)
    ...
    local function UpdatePandemicDisplayControls()
    ...
    hidePandemicCheckbox = UI.CreateModernCheckbox(
        scrollChild,
        L["Hide Blizzard's Pandemic Indicator"],
        CDM.db.hidePandemicIndicator or false,
        function(checked)
            CDM.db.hidePandemicIndicator = checked
            UpdatePandemicDisplayControls()
            API:Refresh("STYLE")
        end
    )
    hidePandemicCheckbox:SetPoint("TOPLEFT", pandemicDisplayHeader, "BOTTOMLEFT", 0, -12)
    ...
    enableCustomizationCheckbox:SetPoint("TOPLEFT", hidePandemicCheckbox, "BOTTOMLEFT", 0, -8)
    ...
    pandemicBorderCheckbox:SetPoint("TOPLEFT", enableCustomizationCheckbox, "BOTTOMLEFT", 0, -18)
    ...
    pandemicBorderColorBuffBarsCheckbox:SetPoint("TOPLEFT", pandemicBorderCheckbox, "BOTTOMLEFT", 0, -8)

    pandemicBorderColor = UI.CreateColorSwatch(scrollChild, L["Color"], "pandemicBorderColor", "STYLE")
    pandemicBorderColor:SetPoint("TOPLEFT", pandemicBorderColorBuffBarsCheckbox, "BOTTOMLEFT", 0, -8)
    ...
    RepositionPandemicSections(CDM.db.glowType or "proc")
end
