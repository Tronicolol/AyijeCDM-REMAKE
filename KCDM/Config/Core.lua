...
local function RunDatabaseMigrations()
    local globalData = KCDMDB.global
...
local function InitializeDB()
    if not KCDMDB then KCDMDB = {} end
    if not KCDMDB.profileKeys then KCDMDB.profileKeys = {} end
    if not KCDMDB.profiles then KCDMDB.profiles = {} end
    if not KCDMDB.global then KCDMDB.global = {} end
    KCDMDB.global.multiChargeSpells = nil
    if not KCDMDB.specProfiles then KCDMDB.specProfiles = {} end

    RunDatabaseMigrations()

    local charKey = UnitName("player") .. " - " .. GetRealmName()
    local defaultProfile = KCDMDB.global.defaultProfile or "Default"
    local profileName = KCDMDB.profileKeys[charKey] or defaultProfile
    KCDMDB.profileKeys[charKey] = profileName

    if not KCDMDB.profiles[profileName] then
        KCDMDB.profiles[profileName] = {}
    end

    local profile = KCDMDB.profiles[profileName]
    ...
    if not KCDMDB.global.cooldownViewerAutoEnabled then
        KCDMDB.global.cooldownViewerAutoEnabled = {}
    end
    if not KCDMDB.global.cooldownViewerAutoEnabled[charKey] then
        KCDMDB.global.cooldownViewerAutoEnabled[charKey] = true
...
function CDM:GetProfileList()
    local list = {}
    for name in pairs(KCDMDB.profiles) do
        list[#list + 1] = name
    end
    table.sort(list)
    return list
...
local function CommitActiveProfileReference(name)
    if not KCDMDB.profiles[name] then
        return false
    end
    KCDMDB.profileKeys[CDM.charKey] = name
    CDM.db = KCDMDB.profiles[name]
...
local function RebuildOptionsIfLoaded(targetTab)
    if not (C_AddOns and C_AddOns.IsAddOnLoaded and C_AddOns.IsAddOnLoaded("KCDM_Options")) then
        return
    end
...
function CDM:ApplyProfile(name, preparedProfileData, options)
    ...
    if not KCDMDB or not KCDMDB.profiles or not KCDMDB.profileKeys then
    ...
    if sourceProfileData == nil then
        sourceProfileData = KCDMDB.profiles[name]
    end
    if sourceProfileData == nil then
    ...
    KCDMDB.profiles[name] = commitProfileData
    CommitActiveProfileReference(name)

    if options.updateCurrentSpecProfile then
        local specData = KCDMDB.specProfiles
            and KCDMDB.specProfiles[self.charKey]
...
function CDM:NewProfile(name)
    ...
    if KCDMDB.profiles[name] then return false, "profile_exists" end
...
function CDM:CopyProfile(sourceName)
    if not KCDMDB.profiles[sourceName] then return false, "profile_not_found" end
    if sourceName == self.activeProfileName then return false, "source_is_active" end
    local source = KCDMDB.profiles[sourceName]
...
function CDM:DeleteProfile(name)
    ...
    if not KCDMDB.profiles[name] then return false, "profile_not_found" end
    local fallback = KCDMDB.global.defaultProfile or "Default"
    if fallback == name then fallback = "Default" end

    KCDMDB.profiles[name] = nil
    for charKey, profileName in pairs(KCDMDB.profileKeys) do
        if profileName == name then
            KCDMDB.profileKeys[charKey] = fallback
        end
    end
    if KCDMDB.specProfiles then
        for _, specData in pairs(KCDMDB.specProfiles) do
            for i = 1, 4 do
    ...
    if KCDMDB.global.defaultProfile == name then
        KCDMDB.global.defaultProfile = "Default"
    end
    return true
end
...
function CDM:RenameProfile(newName)
    ...
    if KCDMDB.profiles[newName] then return false, "profile_exists" end
    ...
    if not KCDMDB.profiles[oldName] then return false, "profile_not_found" end

    local renamedProfileData = KCDMDB.profiles[oldName]

    KCDMDB.profiles[newName] = KCDMDB.profiles[oldName]
    KCDMDB.profiles[oldName] = nil
    for charKey, profileName in pairs(KCDMDB.profileKeys) do
        if profileName == oldName then
            KCDMDB.profileKeys[charKey] = newName
        end
    end
    if KCDMDB.specProfiles then
        for _, specData in pairs(KCDMDB.specProfiles) do
    ...
    if KCDMDB.global.defaultProfile == oldName then
        KCDMDB.global.defaultProfile = newName
    end

    if self.db == renamedProfileData then
        self.db = KCDMDB.profiles[newName]
    end
...
function CDM:GetSpecProfileData()
    if not KCDMDB.specProfiles then return nil end
    return KCDMDB.specProfiles[self.charKey]
end
...
function CDM:SetSpecProfileEnabled(enabled)
    if not KCDMDB.specProfiles then KCDMDB.specProfiles = {} end
    local data = KCDMDB.specProfiles[self.charKey]
    if enabled then
        if not data then
            data = {}
            KCDMDB.specProfiles[self.charKey] = data
        end
        data.enabled = true
...
function CDM:CheckSpecProfileSwitch(specIndex)
    ...
    if not KCDMDB.profiles[targetProfile] then return end
...
local function OpenConfigNow(targetTab)
    if not C_AddOns.IsAddOnLoaded("KCDM_Options") then
        local loaded, reason = C_AddOns.LoadAddOn("KCDM_Options")
        if not loaded then
