...
function API:ExportProfile(profileKey)
    if not KCDMDB or not KCDMDB.profiles then return nil end
    local profile = KCDMDB.profiles[profileKey]
...
function API:GetProfileKeys()
    local keys = {}
    if KCDMDB and KCDMDB.profiles then
...
function API:OpenConfig()
    ...
    if not C_AddOns.IsAddOnLoaded("KCDM_Options") then
        local loaded = C_AddOns.LoadAddOn("KCDM_Options")
...
function API:CloseConfig()
    local frame = _G["KCDMConfigFrame"]
