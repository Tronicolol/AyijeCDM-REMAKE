...
-- Enable with /kcdmdiag on, reproduce the issue, then /kcdmdiag dump.
...
local function PrintActiveFrameList()
    ...
    ForEachCooldownFrame(function(frame, viewerName)
        ...
        if #candidates > 0 then
            found = found + 1
            print("|cffd8c67a[KCDM-LIST]|r " .. tostring(viewerName) .. " cand[" .. CandidateText(candidates) .. "]")
        end
    end)
    if found == 0 then
...
SLASH_KCDMSTRICTDIAG1 = "/kcdmdiag"
SlashCmdList.KCDMSTRICTDIAG = function(message)
    ...
    if command == "" or command == "on" then
        ...
        CDM.Print("Reproduce el fallo y después escribe /kcdmdiag dump")
        return
    end
    ...
    if command == "dump" then
        ...
        if not matchedEver then
            CDM.Print("Target frame was not matched. Run /kcdmdiag list and send that screenshot too.")
        end
        for i = first, count do
            print("|cffd8c67a[KCDM-DIAG]|r " .. lines[i])
        end
        return
    end
    ...
    CDM.Print("Uso: /kcdmdiag on | off | dump | clear | list | <spellID>")
end
