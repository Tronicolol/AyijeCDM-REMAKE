...
local function ShowCustomGlow(frame, overrideColor, overrideType)
    ...
    local selectedType = overrideType or glowCache.type
    if not glowStartFunctions[selectedType] then
        selectedType = "proc"
    end

    if frame.cdmGlowActive and frame.cdmGlowType == selectedType
       and frame.cdmGlowOverrideType == overrideType
    ...
    local fn = glowStartFunctions[selectedType]
    if fn then
        fn(frame, 5, overrideColor)
        frame.cdmGlowActive = true
        frame.cdmGlowType = selectedType
        frame.cdmGlowOverrideType = overrideType
...
local function EnsurePandemicGlowHostFrame(frame)
    if not frame then return nil end
    ...
    frame.cdmPandemicGlowHost = host
    frame.cdmPandemicGlowHostAnchorTarget = nil
    frame.cdmPandemicGlowHostStrata = nil
    frame.cdmPandemicGlowHostLevel = nil
    return host
end

local function SyncPandemicGlowHostFrame(frame, host)
    if not frame or not host then return end

    if frame.cdmPandemicGlowHostAnchorTarget ~= frame then
        host:SetParent(frame)
        host:ClearAllPoints()
        host:SetPoint("TOPLEFT", frame, "TOPLEFT", 0, 0)
        host:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", 0, 0)
        frame.cdmPandemicGlowHostAnchorTarget = frame
    end

    local strata = frame:GetFrameStrata()
    if strata and frame.cdmPandemicGlowHostStrata ~= strata then
        host:SetFrameStrata(strata)
        frame.cdmPandemicGlowHostStrata = strata
    end

    local level = frame:GetFrameLevel()
    if level then
        level = level + 6
        if frame.cdmPandemicGlowHostLevel ~= level then
            host:SetFrameLevel(level)
            frame.cdmPandemicGlowHostLevel = level
        end
    end
end
...
function Glow:ShowPandemicGlow(frame, glowType, color)
    ...
    SyncPandemicGlowHostFrame(frame, host)
    frame.cdmPandemicGlowWanted = true
    frame.cdmPandemicGlowType = glowType or "button"
    frame.cdmPandemicGlowColor = color

    if frame:IsShown() then
        host:Show()
        ShowCustomGlow(host, color, frame.cdmPandemicGlowType)
    end
end
...
function Glow:RefreshActiveGlows()
    ...
    for i = 1, count do
        ...
        if frame.cdmGlowActive then
            local stopFn = glowStopFunctions[frame.cdmGlowType]
            if stopFn then stopFn(frame) end
            local overrideType = frame.cdmGlowOverrideType
            frame.cdmGlowActive = false
            frame.cdmGlowType = nil
            frame.cdmGlowOverrideType = nil
            ShowCustomGlow(frame, frame.cdmGlowOverrideColor, overrideType)
        else
            activeGlowFrames[frame] = nil
        end
    end
